...
func ParseMap(reader io.Reader) (*Map, error) {
	...
	dbg := func(msg string) { if debugOn { fmt.Printf("[parser] @%d %s\n", qt.Position(), msg) } }
	dbg("after header")
 ...
 if envCount, err := qt.ReadInt32(); err == nil && envCount >= 0 && envCount < 100000 {
	...
	if cnt, err := qt.ReadInt32(); err == nil {
		for i := int32(0); i < cnt; i++ {
			...
			if _, err := qt.ReadInt8(); err != nil { break }
			for j := 0; j < 5; j++ { if _, err := qt.ReadUInt16(); err != nil { break } }
		}
	}
	// 4) mpRoomDbHashToRoomId QMap<QString,QUInt> - skip
	if cnt, err := qt.ReadInt32(); err == nil {
		for i := int32(0); i < cnt; i++ {
			if _, err := qt.ReadQString(); err != nil { break }
			if _, err := qt.ReadUInt32(); err != nil { break }
		}
	}
	...
	if cnt, err := qt.ReadInt32(); err == nil {
		for i := int32(0); i < cnt; i++ {
			...
			// value MudletArea
			// QList<QUInt>
			if l, err := qt.ReadInt32(); err == nil { for j := int32(0); j < l; j++ { _, _ = qt.ReadUInt32() } }
			// QList<QInt>
			if l, err := qt.ReadInt32(); err == nil { for j := int32(0); j < l; j++ { _, _ = qt.ReadInt32() } }
			// QMultiMap<int,QPair<int,int>>
			if l, err := qt.ReadInt32(); err == nil { for j := int32(0); j < l; j++ { _, _ = qt.ReadInt32(); _, _ = qt.ReadInt32(); _, _ = qt.ReadInt32() } }
			// gridMode bool
			_, _ = qt.ReadBool()
			// six ints
			for k := 0; k < 6; k++ { _, _ = qt.ReadInt32() }
			// QVector (3 doubles)
			for k := 0; k < 3; k++ { _, _ = qt.ReadDouble() }
			// 4x QMap<int,int>
			for k := 0; k < 4; k++ { if n, err := qt.ReadInt32(); err == nil { for t := int32(0); t < n; t++ { _, _ = qt.ReadInt32(); _, _ = qt.ReadInt32() } } }
			// QVector
			for k := 0; k < 3; k++ { _, _ = qt.ReadDouble() }
			// isZone bool, zoneAreaRef int
			_, _ = qt.ReadBool(); _, _ = qt.ReadInt32()
			// userData QMap<QString,QString>
			if n, err := qt.ReadInt32(); err == nil { for t := int32(0); t < n; t++ { _, _ = qt.ReadQString(); _, _ = qt.ReadQString() } }
		}
	}

	// 10) mRoomIdHash QMap<QString,QInt> - skip
	if cnt, err := qt.ReadInt32(); err == nil { for i := int32(0); i < cnt; i++ { _, _ = qt.ReadQString(); _, _ = qt.ReadInt32() } }

	dbg("before labels")
	// 11) labels: MudletLabels - skip (can be heavy due to embedded PNGs)
	if os.Getenv("MAPSNAP_SKIP_LABELS") == "1" {
		dbg("skipping labels heuristically")
		if err := skipToRoomsHeuristic(qt, m.Areas); err != nil {
			return nil, fmt.Errorf("skipToRoomsHeuristic: %w", err)
		}
	} else if cnt, err := qt.ReadInt32(); err == nil {
		for i := int32(0); i < cnt; i++ {
			if total, err := qt.ReadInt32(); err == nil {
				...
				for j := int32(0); j < total; j++ { // label entries
					...
					for k := 0; k < 7; k++ { _, _ = qt.ReadDouble() }
					_, _ = qt.ReadQString() // text
					// fgColor, bgColor
					_, _ = qt.ReadInt8(); for c := 0; c < 5; c++ { _, _ = qt.ReadUInt16() }
					_, _ = qt.ReadInt8(); for c := 0; c < 5; c++ { _, _ = qt.ReadUInt16() }
					// QPixMap: read header marker (uint32), then check next 4 bytes for PNG magic and consume until IEND
					_, _ = qt.ReadUInt32()
					...
					_, _ = qt.ReadBool(); _, _ = qt.ReadBool()
				}
			}
		}
	}
...
func skipPNG(qt *BinaryReader) error {
	...
	for {
		...
		if buf[0] == needle[0] && buf[1] == needle[1] && buf[2] == needle[2] && buf[3] == needle[3] {
			...
			if err := qt.Skip(8); err != nil { return err }
			return nil
		}
		// advance by one byte and continue
		if _, err := qt.ReadByte(); err != nil { return err }
	}
}
...
func skipToRoomsHeuristic(qt *BinaryReader, areas map[int32]*Area) error {
	...
	for id := range areas { valid[id] = struct{}{} }
	// Sliding window over bytes, checking 8-byte sequences as two int32
	for {
		peek, err := qt.Peek(8)
		if err != nil || len(peek) < 8 { return fmt.Errorf("EOF before rooms") }
		...
		if id > 0 {
			if _, ok := valid[area]; ok {
				// Found plausible start of a room entry. Do not consume bytes; let parseRooms proceed from here.
				return nil
			}
		}
		// advance by one byte
